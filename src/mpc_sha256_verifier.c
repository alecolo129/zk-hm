...
int mpc_sha256_verify(ZkBooCommit *a, int e, ZkBooOpen *z) {
  int ret = -1;
  unsigned char *randomness_block = NULL;
  unsigned char *randomness[2] = {NULL, NULL};
  uint8_t *padded[2] = {NULL, NULL};

  if (!a || !z || !z->ve || !z->ve1 || e < 0 || e >= 3) {
    LOG_ERRF("Invalid mpc_sha256_verify input");
    return -1;
  }

  uint32_t result[8];
  output(z->ve, result);
  if (memcmp(a->yp[e], result, sizeof(result)) != 0) {
    LOG_ERRF("Committed outputs inconsistent with opening");
    goto cleanup;
  }

  output(z->ve1, result);
  if (memcmp(a->yp[(e + 1) % 3], result, sizeof(result)) != 0) {
    LOG_ERRF("Committed outputs inconsistent with opening");
    goto cleanup;
  }

  randomness_block = malloc(2u * RAND_BYTES);
  if (!randomness_block) {
    LOG_ERRF("malloc failure");
    goto cleanup;
  }
  randomness[0] = randomness_block;
  randomness[1] = randomness_block + RAND_BYTES;
  ...
  padded[0] = calloc(paddedLen, 1);
  padded[1] = calloc(paddedLen, 1);
  if (!padded[0] || !padded[1]) {
    LOG_ERRF("calloc failure");
    goto cleanup;
  }
  memcpy(padded[0], rvec_const_bytes(&z->ve->x), L_BYTES);
  memcpy(padded[1], rvec_const_bytes(&z->ve1->x), L_BYTES);
